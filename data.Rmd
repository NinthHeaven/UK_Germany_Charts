...
# What function should do: Get the genres from each song. Then, I would use a map function and paste the genres onto a new column in the uk2000_genre df.
a_function <- function(list) {
  for(n in 1:length(list)) {
    paste(list[[n]])
  }
}

# Creating dummy df to test it on. 
uk2000_genre <- uk2000_tracks 

# Alternative plan to the function: Create a dataframe where the list items but put into one column, and then add that column to uk2000_tracks.
testdf <- map_dfr(test, ~as.data.frame(t(.x)))
